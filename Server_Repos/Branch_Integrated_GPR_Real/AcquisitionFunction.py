...
class AcquisitionFunction():
    ...
    def expected_improvement_util(self, x, y_max, gp_obj):
        ...
        with np.errstate(divide='ignore') or np.errstate(invalid='ignore'):
            ...
            # Commenting to compute the minimum negative log likelihood
            # return -1 * result
            return result
    ...
    def min_acq_func(self, gp_obj, Xs, ys, iteration_count):
        ...
        if (self.acq_type == 'pi'):
            ...
            for starting_point in starting_points:
                ...
                min_x = opt.minimize(lambda x: self.probability_improvement_util(x, y_min, gp_obj), starting_point,
                                     method='L-BFGS-B',
                                     tol=0.001,
                ...
                std_dev = np.sqrt(variance)
                fvalue = self.probability_improvement(mean, std_dev, y_min)
                ...
                if (fvalue > fmin):
                    PH.printme(PH.p1, "New best Fval: ",fvalue," found at: ", min_x['x'])
                    x_max_value = min_x['x']
                    fmin = fvalue

            PH.printme(PH.p1, "PI Best is ", fmin, "at ", x_max_value)
            ...
            with np.errstate(invalid='ignore'):
                ...
                std_dev = np.sqrt(variance)
                acq_func_values = self.probability_improvement(mean, std_dev, y_min)
        ...
        # Find maxima of the ACQ function using UCB
            ...
            for starting_point in starting_points:
                ...
                min_x = opt.minimize(lambda x: self.upper_confidence_bound_util(x, gp_obj, iteration_count),starting_point ,
                                     method='L-BFGS-B',
                                     tol=0.001,
                                     bounds=gp_obj.bounds)
            ...
            PH.printme(PH.p1, "UCB Best is ", fmin, "at ", x_max_value)
            ...
            # used to verify if the maxima value is really found at 0
            # if(x_max_value==[0]):
            #     PH.printme(PH.p1, tempfvals, tempmax_x)
        ...
        # Find maxima of the ACQ function using EI
            ...
            #Commenting to compute the minimum negative loglikelihood
            # y_min = gp_obj.y.max()
            y_min = gp_obj.y.min()
            PH.printme(PH.p1, "ACQ Function : EI ")

            # Obtain the maxima of the ACQ function by starting the optimization at different start points
            for starting_point in starting_points:

                # Find the maxima in the bounds specified for the PI ACQ function
                min_x = opt.minimize(lambda x: self.expected_improvement_util(x, y_min, gp_obj), starting_point,
                                     method='L-BFGS-B',
                                     tol=0.01,
                ...
                std_dev = np.sqrt(variance)
                fvalue = self.expected_improvement(mean, std_dev, y_min)
            ...
            PH.printme(PH.p1, "EI Best is ", fmin, "at ", x_max_value)

            # Calculate the ACQ function values at each of the unseen data points to plot the ACQ function
            with np.errstate(invalid='ignore'):
                mean, variance, f_prior, f_post = gp_obj.gaussian_predict(Xs)
                std_dev = np.sqrt(variance)
                acq_func_values = self.expected_improvement(mean, std_dev, y_min)
...
# PH.printme(PH.p1, 'value\n' ,value)
# beta3 = 2* np.log(value)
# PH.printme(PH.p1, 'beta3L: ', beta3)
    ...
    #
    #             if (fvalue > fmax):
    #                 PH.printme(PH.p1, "New best Fval: ",fvalue," found at: ", max_x['x'])
    #                 x_max_value = max_x['x']
    #                 fmax = fvalue
    #
    #         PH.printme(PH.p1, "Best is ", fmax, "at ", x_max_value)
    #
    #         with np.errstate(invalid='ignore'):
    #             mean, variance, f_prior, f_post = gp_obj.gaussian_predict(Xs)
    #             std_dev = np.sqrt(variance)
