...
class BayesianOptimization:
    ...
    def run_bayes_opt(self, run_count):
        ...
        PH.printme(PH.p1, "************ BO Run: " + str(
            run_count) + "     ACQ:" + self.acq_func_obj.acq_type.upper() + "    ***************\n\n")
        PH.printme(PH.p1, 'Initial Values for this run\n---X---\n', self.gp_obj.X)
        PH.printme(PH.p1, '---y---\n', self.gp_obj.y, '\n\n')
        ...
        for i in range(self.no_iterations):
            PH.printme(PH.p1, "###### BO run: " + str(run_count) + "  iteration: ", i + 1,"    " ,self.gp_obj.kernel_char,"  ######")
            ...
            if (self.gp_obj.weights_estimation and (i + 1) % 3 == 0):
                ...
                if not self.gp_obj.disp_bool:
                    PH.printme(PH.p1, "!!!!!!!!!!!Matrix went non positive definite, inverting with pinv")
                self.disp_bool = True
            ...
            if (self.acq_func_obj.acq_type == 'rs'):
                ...
                xnew, acq_func_values = self.acq_func_obj.min_acq_func(self.gp_obj, Xs, ys, i + 1)
            ...
            # Commented to plot accuracy
            # ith_regret = self.y_true_max  - y.max()
            # regret = np.append(regret, np.matrix(ith_regret))
            regret = np.append(regret, y.min())
            PH.printme(PH.p1, "\n")

        # Display the final values for this iteration
        PH.printme(PH.p1, 'Final values:\n$X: \n', self.gp_obj.X.T, '\n$y:\n', self.gp_obj.y.T)
        PH.printme(PH.p1, "True Max: ", self.y_true_max)
        PH.printme(PH.p1, self.acq_func_obj.acq_type.upper(), self.gp_obj.kernel_char,", Observed Maximum value: ", self.gp_obj.y.max())
        PH.printme(PH.p1, "Regret: ", regret)
        PH.printme(PH.p1, "\n\n\n")

        # PH.printme(PH.p1, "After Optimization")
...
'''
                ...
                #     # PH.printme(PH.p1, "Initial values for a & b are ",param_a, param_b)
                #     maxima = opt.minimize(lambda x: -self.gp_obj.optimize_log_marginal_likelihood_l_params(x),
                #                           init_points,
                #                           method='L-BFGS-B',
                #                           bounds=total_bounds)
                ...
                #
                # PH.printme(PH.p1, "Opt Params: ", self.gp_obj.len_scale_params)
                #
                # # Recomputing L according to the updated length scale
                # self.gp_obj.L_x_x = self.gp_obj.compute_l(self.gp_obj.X)
