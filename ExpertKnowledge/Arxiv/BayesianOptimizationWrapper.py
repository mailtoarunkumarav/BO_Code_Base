from Arxiv.ScratchGPModular import GaussianProcess
from Arxiv.AcquisitionFunction import AcquisitionFunction
from Arxiv.BayesianOptimization import BayesianOptimization
from Arxiv.Functions import FunctionHelper, Custom_Print
...
class BayesianOptimizationWrapper:
    ...
    def opt_wrapper(start_time):
        ...
        #Benchmark function
        linspacexmin = 0;linspacexmax = 10;linspaceymin = 0;linspaceymax = 2
        ...
        if true_func == 'sin' or true_func == 'cos' or true_func == 'custom':
            ...
            # custom
            Xmin = np.array([0]);Xmax = np.array([10]);ymin = 0;ymax = 2
        ...
        len_scale_bounds = [[0.1, 0.8] for nd in np.arange(number_of_dimensions)]

        # characteristic_length_scale = np.array([1 for nd in np.arange(number_of_dimensions)])
        char_length_scale = [ 0.9 for nd in np.arange(number_of_dimensions)]
        ...
        multi_len_scales = [0.7,0.4,0.3,0.01]
        len_weights = [1,1,1,1]
        ...
        ei_fixed_l_regret = []
        ei_multi_l_regret = []
        ei_free_kernel_regret = []

        # kernel_iter_types = ['var_l', 'm_ker', 'ard', 'fix_l']
        # kernel_iter_types = ['fix_l']
        kernel_iter_types = ['m_ker']
        # kernel_iter_types = ['ard']
        # kernel_iter_types = ['free_ker', 'ard', 'fix_l']
        # kernel_iter_types = ['free_ker']

        # Beta bounds
        beta_bounds = [[-1, 1], [-1, 1]]
        beta_params = [0.5, 0.5]
        free_kernel_betas_estimation = False
        random_vectors=np.array([0.2, 0.5])
        ...
        print("\n\nParameters\nTrue Func", true_func, "\tDim:", number_of_dimensions, "\tobs_samp:", number_of_observed_samples,
              ...
              "beta_params:", beta_params, "beta_bounds:",beta_bounds, "Random Vectors: ",random_vectors,"\n\n")
        ...
        for i in range(number_of_runs):
            ...
            gaussianObject = GaussianProcess(kernel_type, params_estimation, len_scale_estimation, char_length_scale,
                                             ...
                                             Xmin, Xmax, ymin, ymax, len_weight_bounds, len_weights, weights_estimation,
                                             multi_len_scales, beta_bounds, beta_params, free_kernel_betas_estimation, random_vectors)
            ...
            # bay_opt_obj.y_true_max = true_max
            # init_regret = []
            ...
            if ('pi' in acq_fun_list):
                ...
                pi_regret_eachrun = bay_opt_obj.run_bayes_opt(i + 1, start_time)
            ...
            if ('rs' in acq_fun_list):
                ...
                gaussianObject.signal_variance = signal_variance
                gaussianObject.gaussian_fit(X, y)
                rs_regret_eachrun = bay_opt_obj.run_bayes_opt(i + 1, start_time)
            ...
            if ('ei' in acq_fun_list):
                ...
                for iter_type in kernel_iter_types:
                    ...
                    # Commented as it is replaced by other code (specific to Kernel selected)
                    # total_ei_regret.append(ei_regret_eachrun)

                    if(iter_type == 'ard'):
                        print("Testing with ARD Kernel in Run: ", (i+1))
                        gaussianObject.len_scale_estimation = True
                        ei_regret_eachrun =  bay_opt_obj.run_bayes_opt(i + 1, start_time)
                    ...
                    elif (iter_type == 'free_ker'):
                        print("Testing with free Kernel in Run: ", (i + 1))
                        gaussianObject.free_kernel_betas_estimation = True
                        ei_regret_eachrun = bay_opt_obj.run_bayes_opt(i + 1, start_time)
                        ei_regret_eachrun = np.append(init_regret, ei_regret_eachrun)
                        gaussianObject.free_kernel_betas_estimation = False
                        ei_free_kernel_regret.append(ei_regret_eachrun)
            ...
            if ('ucb' in acq_fun_list):
                ...
                gaussianObject.signal_variance = signal_variance
                gaussianObject.gaussian_fit(X, y)
                ucb_regret_eachrun = bay_opt_obj.run_bayes_opt(i + 1, start_time)
        ...
        if ('ei' in acq_fun_list):
            ...
            for iter_type in kernel_iter_types:
                if (iter_type == 'ard'):
                    ...
                    ax.plot(iterations_axes_values, ei_multi_regret_mean, 'y')
                    # plt.errorbar(iterations_axes_values, ei_regret_mean, yerr= ei_regret_std_dev )
                    plt.gca().fill_between(iterations_axes_values, ei_multi_regret_mean + ei_multi_regret_std_dev,
                                           ei_multi_regret_mean - ei_multi_regret_std_dev, color="yellow", alpha=0.25,
                                           label="Multi")

                elif (iter_type == 'free_ker'):
                    ei_free_kernel_regret = np.vstack(ei_free_kernel_regret)
                    ei_free_kernel_regret_mean = np.mean(ei_free_kernel_regret, axis=0)
                    ei_free_kernel_regret_std_dev = np.std(ei_free_kernel_regret, axis=0)
                    ei_free_kernel_regret_std_dev = ei_free_kernel_regret_std_dev / np.sqrt(number_of_iterations + (number_of_dimensions + 1))
                    print("\n\nTotal FREE FIX Regret\n", ei_free_kernel_regret, "\n\nFREE Regret Mean", ei_free_kernel_regret_mean,
                          "\n\nFREE Regret Deviation\n", ei_free_kernel_regret_std_dev)

                    ax.plot(iterations_axes_values, ei_free_kernel_regret_mean, 'y')
                    # plt.errorbar(iterations_axes_values, ei_regret_mean, yerr= ei_regret_std_dev )
                    plt.gca().fill_between(iterations_axes_values, ei_free_kernel_regret_mean + ei_free_kernel_regret_std_dev,
                                           ei_free_kernel_regret_mean - ei_free_kernel_regret_std_dev, color="yellow", alpha=0.25,
                                           label="EFK")
        ...
        plt.axis([1, len(iterations_axes_values), linspaceymin, linspaceymax])
        plt.title('Regret')
    ...
    if __name__ == "__main__":
        timenow = datetime.datetime.now()
        start_time =  timenow.strftime("%H%M%S_%d%m%Y")
        f = open('console_output_'+str(start_time)+'.txt', 'w')
        original = sys.stdout
        sys.stdout = Custom_Print(sys.stdout, f)
        opt_wrapper(start_time)
