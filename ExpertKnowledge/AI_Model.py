...
class AIModel:
    ...
    def kernel_optimisation_with_ai_suggestions(self, plot_files_identifier, gp_aimodel, acq_func_obj, noisy_suggestions,
        ...
        if self.global_opt_method == "SLSQP":
            if gp_aimodel.kernel_type == 'SE':
                nlc1 = NonlinearConstraint(lambda x: self.kernel_correction(x, gp_aimodel, acq_func_obj, noisy_suggestions,
                                                                            ai_suggestion_count), 0, np.inf)
                constraint_bounds = [[0.05, 1] for i in range(gp_aimodel.number_of_dimensions+1)]

            elif gp_aimodel.kernel_type == 'MKL':
                nlc1 = NonlinearConstraint(lambda x: self.kernel_correction(x, gp_aimodel, acq_func_obj, noisy_suggestions,
                                                                            ai_suggestion_count), 0, np.inf)
                constraint_bounds = [[0.05, 1] for i in range(6)]
    ...
    def kernel_correction(self, x, gp_aimodel, acq_func_obj, noisy_suggestions, ai_suggestion_count):
        ...
        for i in range(len(gp_aimodel.he_suggestions["x_suggestions_best"])):
            ...
            if acq_func_obj.acq_type == "ucb":
                ...
                acq_value_he = acq_func_obj.data_conditioned_upper_confidence_bound_util("ai", noisy_suggestions,
                ...
                acq_value_ai = acq_func_obj.data_conditioned_upper_confidence_bound_util("ai", noisy_suggestions,
                                                                                           gp_aimodel.ai_suggestions[current_index],
                                                                                           data_conditioned_on_current_he_suggestion_X,
                                                                                           data_conditioned_on_current_he_suggestion_y,
                                                                                           gp_aimodel, ai_suggestion_count)
            if acq_func_obj.acq_type == "ei":
                y_max = gp_aimodel.y.max()
                acq_value_he = acq_func_obj.data_conditioned_expected_improvement_util("ai", noisy_suggestions,
                                                                                         gp_aimodel.he_suggestions[
                ...
                acq_value_ai = acq_func_obj.data_conditioned_expected_improvement_util("ai", noisy_suggestions,
                                                                                       gp_aimodel.X[current_index-1],
                                                                                       data_conditioned_on_current_he_suggestion_X,
                                                                                       data_conditioned_on_current_he_suggestion_y,
                                                                                       y_max, gp_aimodel)

            acq_diff_value = acq_value_he - acq_value_ai
    ...
    def obtain_twostg_aimodel_suggestions(self, plot_files_identifier, gp_aimodel, acq_func_obj, noisy_suggestions, ai_suggestion_count,
        ...
        if gp_aimodel.he_suggestions is not None:
            ...
            #                                     start_points_list, log_like_max)
    ...
    def distance_maximiser_for_likelihood(self, gp_aimodel, acq_func_obj, noisy_suggestions, ai_suggestion_count, stage_one_best_kernel,
        ...
        # constraint_list = []
        # const_llk = {'type': 'ineq', 'fun': lambda x: gp_aimodel.optimize_log_marginal_likelihood_l(x)[0] - compromised_likelihood}
        # const_cond = {'type': 'ineq', 'fun': lambda x: -gp_aimodel.get_kernel_matrix_condition_number(x)+25000}
        #
        # constraint_list.append(const_llk)
        # constraint_list.append(const_cond)
        ...
        #                       )
        # # # # SHGO
        # # XXXXXXXXXXX SE Based - XXXXXXXXXX
        # total_bounds = [(0.1, 1.0)]
        # # constraints = ({'type': 'ineq', 'fun': lambda x: self.ineq_llk_constraint1(x, empty_gradient_info, gp_aimodel,
        # #                                                                            compromised_likelihood)},
        # #                {'type': 'ineq', 'fun': lambda x: self.ineq_llk_constraint2(x, empty_gradient_info, gp_aimodel,
        # #                                                                         condition_num_max)}
        # #                )
        #
        # maxima = opt.shgo(lambda x: -self.constrained_distance_maximiser(x, empty_gradient_info, gp_aimodel, acq_func_obj,
        #                    noisy_suggestions, ai_suggestion_count), total_bounds,
        #                   # n=100, iters=4,
        #                    options={'disp': True})
        # params = maxima['x']
    ...
    def constrained_distance_maximiser(self, inputs, grad, gp_aimodel, acq_func_obj, noisy_suggestions, ai_suggestion_count):
        ...
        for i in range(len(gp_aimodel.he_suggestions["x_suggestions_best"])):
            ...
            # mean = np.mean(random_acq_values)
            # if mean != 0:
            #     best_acq_value_mean = best_acq_value / mean
            #     worst_acq_value_mean = worst_acq_value / mean
