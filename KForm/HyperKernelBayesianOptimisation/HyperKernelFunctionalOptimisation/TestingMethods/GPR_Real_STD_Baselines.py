...
class GaussianProcess:
    ...
    def __init__(self, output_gen_time, kernel_type, number_of_test_datapoints, noise, random_seed, linspacexmin, linspacexmax,
                 ...
                 weight_params_estimation, degree_estimation, degree, ARD_lengthscale_bounds):
        ...
        self.degree = degree
        self.ARD_lengthscale_bounds = ARD_lengthscale_bounds
    ...
    def computekernel(self, data_point1, data_point2):
        ...
        if self.kernel_type == 'ARD':
            # PH.printme(PH.p1, "SE Kernel")
             result = self.ard_sq_exp_kernel(data_point1, data_point2, self.char_len_scale, self.signal_variance)
    ...
    def optimize_log_marginal_likelihood_l(self, input):
        ...
        init_charac_length_scale = np.array(input[0])
        signal_variance = input[1]
        ...
        if self.kernel_type == 'SE':
            ...
            K_x_x = self.ard_sq_exp_kernel(self.X, self.X, init_charac_length_scale, signal_variance)
        elif self.kernel_type == 'MATERN3':
    ...
    def compute_predictive_likelihood(self, count, kernel_type, observations_kernel):
        ...
        for i in range(len(self.Xs)):
            ...
            # PH.printme(PH.p1,"Likelihood: ", likeli)
            if likeli == 0:
                PH.printme(PH.p1, "\n\n\n\nMean: ", mean, "\nVariance: ", variance, "\nys: ", self.ys.shape, ".....\n")
                PH.printme(PH.p1,"Error occured in calculating likelihood ")
            neg_log_likelihood = -1 * np.log(likeli)
            neg_log_sum = neg_log_sum + neg_log_likelihood
            # PH.printme(PH.p1,"prob: ", likeli)
            # PH.printme(PH.p1,"-log: ", neg_log_likelihood)
        # PH.printme(PH.p1,"sum_-log: ", neg_log_sum)
    ...
    def runGaussian(self, count, kernel_type, observations_kernel, optional_hyper_gp_obj):
        ...
        if self.hyper_params_estimation:
            ...
            PH.printme(PH.p1, "SE Hyper Params estimating..")
            ...
            starting_points = np.random.uniform(self.lengthscale_bounds[0],
                                                               self.lengthscale_bounds[1],
                                                               self.number_of_restarts_likelihood)
            ...
            total_bounds = []
            total_bounds.append(self.lengthscale_bounds)
            ...
            for ind in np.arange(self.number_of_restarts_likelihood):
                ...
                len_scale_temp = maxima['x'][0]
                variance_temp = maxima['x'][1]
                ...
                if (log_likelihood > log_like_max):
                    PH.printme(PH.p1, "New maximum log likelihood ", log_likelihood, " found for l= ",
                          maxima['x'][0], " var:", maxima['x'][1])
                    x_max_value = maxima
                    log_like_max = log_likelihood

            self.char_len_scale = x_max_value['x'][0]
            self.signal_variance = x_max_value['x'][1]
            PH.printme(PH.p1, "Opt Length scale: ", self.char_len_scale, "\nOpt variance: ", self.signal_variance)


        elif self.ARD_hyper_params_estimation:

            PH.printme(PH.p1, "ARD Hyper Params estimating..")
            # Estimating Length scale itself
            x_max_value = None
            ...
            # Depending on the number of dimensions and bounds, generate random multi-starting points to find maxima
            for dim in np.arange(self.number_of_dimensions):
                random_data_point_each_dim = np.random.uniform(self.ARD_lengthscale_bounds[dim][0],
                                                               self.ARD_lengthscale_bounds[dim][1],
                                                               self.number_of_restarts_likelihood). \
                    reshape(1, self.number_of_restarts_likelihood)
                random_points.append(random_data_point_each_dim)
            ...
            total_bounds = self.ARD_lengthscale_bounds.copy()
            total_bounds.append(self.signal_variance_bounds)
        ...
        #
        # # Cholesky decomposition to find L from covariance matrix K i.e K = L*L.T
        # L_xs_xs = np.linalg.cholesky(K_xs_xs + 1e-12 * np.eye(len(self.Xs)))
        #
        # # Sample 3 standard normals for our test points
        # standard_normals = np.random.normal(size=(len(self.Xs), 3))
        #
        ...
        PH.printme(PH.p1, "calculating log loss")
        mean, variance, factor1 = self.compute_mean_var(self.Xs, self.X, self.y)
        ...
        for i in range(len(self.Xs)):
            ...
            print(mean[i], variance[i], ys[i])

            # neg_log_likelihood = 0.5*np.log(2*np.pi*variance[i]) + ((0.5/variance[i])*((self.ys[i]-mean[i])**2))
            # print(neg_log_likelihood)

            likeli = (1 / (np.sqrt(2 * np.pi * variance[i]))) * (np.exp((-1 / (2 * variance[i])) * ((self.ys[i] - mean[i]) ** 2)))
            # PH.printme(PH.p1,"Likelihood: ", likeli)
            # if likeli == 0:
            #     c = ((self.ys[i] - mean[i]) ** 2)
            #     d = (-1 / (2 * variance[i]))
            #     v = (np.exp(d* c))
            #     a = (1 / (np.sqrt(2 * np.pi * variance[i])))
            #     PH.printme(PH.p1, "\n\n\n\nMean: ", mean, "\nVariance: ", variance, "\nys: ", self.ys.shape, ".....\n")
            #     PH.printme(PH.p1, "Error occured in calculating likelihood ")
            neg_log_likelihood = -1 * np.log(likeli)
            neg_log_sum = neg_log_sum + neg_log_likelihood
...
if __name__ == "__main__":
    ...
    if dataset == 'fertility':
        ...
        ARD_lengthscale_bounds = [[0.1, 1] for i in range(0, number_of_dimensions)]
        # char_len_scale = [0.2 for i in range(0, number_of_dimensions)]
        # char_len_scale = [0.2]
        ...
        total_data_std=total_data

        D = total_data_std[:, 0:-1].to_numpy()
        f = total_data_std[:, -1].to_numpy()
    ...
    # Commenting to add real GPR code
    # # random_points = []
    # # X = []
    # #
    ...
    gaussianObject = GaussianProcess(str(stamp), kernel_type, number_of_test_datapoints, noise, random_seed, linspacexmin,
                                     ...
                                     degree_estimation, degree, ARD_lengthscale_bounds)
    count = 1
    ...
    for kernel in kernel_types:
        ...
        if kernel == 'MKL':
            ...
            gaussianObject.hyper_params_estimation = False
            gaussianObject.degree_estimation = False
            gaussianObject.ARD_hyper_params_estimation = True
        ...
        y_pred, std = model.predict(Xs, return_std=True)
        print(y_pred,"\n", params)
        ...
        for i in range(len(Xs)):
            ...
            likeli = (1 / (np.sqrt(2 * np.pi * (std[i]**2)))) * (np.exp((-1 / (2 * (std[i]**2))) * ((ys[i] - y_pred[i]) ** 2)))
            PH.printme(PH.p1,"Likelihood: ", likeli)
            if likeli == 0:
                c = ((ys[i] - y_pred[i]) ** 2)
                d = (-1 / (2 * (std[i]**2)))
                v = (np.exp(d* c))
                a = (1 / (np.sqrt(2 * np.pi * (std[i]**2))))
                PH.printme(PH.p1, "Error occured in calculating likelihood ")
            neg_log_likelihood = -1 * np.log(likeli)
            # print(neg_log_likelihood, neg_log_sum)
            neg_log_sum = neg_log_sum + neg_log_likelihood
