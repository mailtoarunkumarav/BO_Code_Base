...
sys.path.insert(0, "C:\Arun_Stuff\GitHubCodes\KForm\HyperKernelBayesianOptimisation\HyperKernelFunctionalOptimisation"
                   "\KFO_Krien_EVD_KernelRecovery_NeurIPS_Rebuttal\GP_Regressor")
...
class KernelOptimizationWrapper:
    ...
    def kernel_wrapper(self, start_time, input):
        ...
        hyper_lambda = 0.0125408
        hyper_char_len_scale = 0.631474
        ...
        PH.printme(PH.p1, "Configuration Settings:\n\n", "hyperkernel_type:", hyperkernel_type, "\tkernel_type:", kernel_type,
                   ...
                   number_of_restarts_likelihood, "\thyper_char_len_scale: ", hyper_char_len_scale,"\tNo principal Components: ",
        ...
        for i in range(number_of_runs):
            ...
            if synthetic_data:
                posterior_plot = True
                PH.printme(PH.p1, "Constructing ground truth kernel for the first time")
                role = "ground_truth_gp"
                gp_wrapper_obj_groundtruth = GaussianProcessWrapper(synthetic_data)
                groundtruth_gp = gp_wrapper_obj_groundtruth.construct_gp_regressor_syn(start_time, posterior_plot, role)

                role = "gp_sampling"
                gp_wrapper_obj.groundtruth_gp = groundtruth_gp
                gp_wrapper_obj.construct_gp_regressor_syn(start_time, posterior_plot, role)
