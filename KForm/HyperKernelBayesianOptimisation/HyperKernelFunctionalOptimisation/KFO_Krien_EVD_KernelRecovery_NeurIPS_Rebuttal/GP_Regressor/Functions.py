...
class FunctionHelper:
    ...
    def get_true_max(self):
        ...
        elif (self.true_func_type == "gp_sampling"):
            true_max = self.get_true_func_value(2.0202)

        elif (self.true_func_type == "ground_truth_gp"):
            true_max = self.get_true_func_value(2.0202)
    ...
    def get_true_func_value(self, x):
        if (self.true_func_type == 'sin'):
            ...
            mean, variance, fprior, f_post = self.groundtruth_gp.gaussian_predict(np.array([x]))
            return mean

        elif self.true_func_type == "ground_truth_gp":

            value = np.exp(-(x - 2) ** 2) + np.exp(-(x - 6) ** 2 / 10) + 1 / (x ** 2 + 1)
            value = (value -0.8644480208027399 )/0.2757906602609252
            return value
