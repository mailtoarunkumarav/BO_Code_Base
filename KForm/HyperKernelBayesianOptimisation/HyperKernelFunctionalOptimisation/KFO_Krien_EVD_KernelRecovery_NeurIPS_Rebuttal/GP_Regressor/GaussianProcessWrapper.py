...
class GaussianProcessWrapper:
    ...
    def construct_gp_regressor_real(self, stamp, dataset, posterior_plot):
        ...
        true_func_type = "ground_truth_gp"
    ...
    def construct_gp_regressor_syn(self, stamp, posterior_plot, role):
        ...
        if role == "gp_sampling":
            true_func_type = "gp_sampling"
            number_of_observed_samples = 40
            func_helper_obj = FunctionHelper(true_func_type)
            func_helper_obj.groundtruth_gp = self.groundtruth_gp

        elif role == "ground_truth_gp":
            true_func_type = "ground_truth_gp"
 ...
 #        X = np.vstack(X)
        ...
        PH.printme(PH.p1, role, "Data points: ",X)
        ...
        #  Commenting as Benchmark function is normalised at source
        # X = np.divide((X - Xmin), (Xmax - Xmin))
        ...
        if role == "ground_truth_gp":
            gaussian_object.runGaussian(0, "MATERN3", None, None)
