...
class NormUtility:
    ...
    def inifinity_norm_calculator_csv_experiment1(self, filename1, filename2):
        ...
        PH.printme(PH.p1, "Reading File 2 : groundtruth kernel")
        with open(filename2) as file_name:
            array = np.loadtxt(file_name, delimiter=",")
        kernel_gt = array.reshape(-1, 1)

        PH.printme(PH.p1, "kfo: ", kernel_kfo.shape, "   GT", kernel_gt.shape)
        kernel_diff = kernel_kfo - kernel_gt

        inf_norm = np.linalg.norm(kernel_diff, ord=np.inf, axis=0)
        l2norm = np.linalg.norm(kernel_diff, ord=2, axis=0)

        PH.printme(PH.p1, "Experiment2: KFO vs Ground Truth Kernel \nInfinity norm Difference: ", inf_norm,
...
if __name__ == "__main__":
    ...
    norm = NormUtility()
    norm.inifinity_norm_calculator_csv_experiment1("Kernel_KFO_Exp2.csv", "Kernel_GT_Exp2.csv")
    timenow = datetime.datetime.now()
