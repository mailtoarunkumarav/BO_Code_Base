...
class GaussianProcess:
    ...
    def compute_rmse(self, count, kernel_type, observations_kernel, optional_hyper_gp_obj):
        ...
        for i in range(len(self.Xs)):
            # likeli = (1 / (np.sqrt(2 * np.pi * variance[i]))) * (np.exp((-1 / (2 * variance[i])) * ((self.ys[i] - mean[i]) ** 2)))
            # rmse = np.sqrt((self.ys[i] - mean[i]) ** 2) * (1 / self.number_of_test_datapoints)
            rmse = ((self.ys[i] - mean[i]) ** 2) * (1 / self.number_of_test_datapoints)
            # PH.printme(PH.p1,"RMSE: ", rmse)
            rmse_sum = rmse_sum + rmse
            # PH.printme(PH.p1,"RMSE_SUM: ", rmse_sum)
        ...
        print("this is rmse sum", rmse_sum)
        return rmse_sum
