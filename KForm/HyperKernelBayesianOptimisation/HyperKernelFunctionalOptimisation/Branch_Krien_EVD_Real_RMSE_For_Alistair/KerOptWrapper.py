...
sys.path.insert(0, 'C:\Arun_Stuff\GitHubCodes\KForm\HyperKernelBayesianOptimisation\HyperKernelFunctionalOptimisation'
                   '\Branch_Krien_EVD_Real_RMSE_For_Alistair\GP_Regressor')
...
class KernelOptimizationWrapper:
    ...
    def kernel_wrapper(self, start_time, input):
        ...
        for i in range(number_of_runs):
            ...
            if synthetic_data:
                ...
                for i in range(10):
                    ...
                    rmse = gp_wrapper_obj.gaussian_object.compute_rmse("", "", "", "")
                    rmse_total = np.append(rmse_total, rmse)
                print("Dataset: ", dataset ,"\tMean: ", rmse_total.mean(), "\t:StdDev:",np.std(rmse_total))
                exit(0)
            ...
            if 'UCB' in acq_fun_list:
                ...
                # Negative Likelihood Calculations
                ...
                minimum_rmse = gp_wrapper_obj.compute_rmse_for_kernel('HYPER', best_solution_found['best_kernel'],
                                                                                  hyper_gaussian_object)
                PH.printme(PH.p1, "Minimum RMSE obtained: ", minimum_rmse)
                tot_min_rmse = np.append(tot_min_rmse, minimum_rmse)
        ...
        mean_min_rmse = np.mean(tot_min_rmse)
        std_min_rmse = np.std(tot_min_rmse)

        PH.printme(PH.p1, "\n\n@@@@@@@Mean likelihood for Bayesian Optimisation : ", mean_min_rmse)
        PH.printme(PH.p1, "Std: ", std_min_rmse)
        # # Uncomment to see the graph if not integrated with BO
        # plt.show()
        return mean_min_rmse
