...
class GaussianProcess:
    ...
    def gaussian_fit(self,X, y):
        ...
        PH.printme(PH.p1, "L Recaluated with new data")
    ...
    def computekernel(self, data_point1, data_point2):
        ...
        # Kernel_type = 3 represents the Periodic Kernel***(To be analysed for fixin the parameters)
            ...
            PH.printme(PH.p1, "Periodic Kernel")
            # self.charac_length_scale = 0.1
    ...
    def sq_exp_kernel(self, data_point1, data_point2, char_len_scale, signal_variance):
        ...
        if( self.kernel_char == 'ard' or self.kernel_char == 'fix_l'):
            ...
            #     PH.printme(PH.p1, "ARD kernel is set for the computations")
            # else:
            # PH.printme(PH.p1, "Fixed length kernel is set for the computations")
            return self.ard_sq_exp_kernel(data_point1, data_point2, char_len_scale, signal_variance)

        elif(self.kernel_char == 'var_l'):
            # PH.printme(PH.p1, "Var kernel is set for the computations")
    ...
    def len_scale_func_quad(self, data_point_value, len_scale):
        ...
        if value == 0:
            value = 1e-6
        if value < 0 :
            PH.printme(PH.p1, "quad length scale value is less than zero")
        return value
    ...
    def var_sq_exp_kernel(self, data_point1, data_point2, char_len_scale, signal_variance):
        ...
        for i in np.arange(len(data_point1)):
            for j in np.arange(len(data_point2)):
                ...
                if(total_product < 0):
                    PH.printme(PH.p1, "Product term of length scale is less than zero", data_point1, data_point2)
    ...
    def optimize_log_marginal_likelihood_weight_params(self, input):
        ...
        try:
            ...
            PH.printme(PH.p1, "Matrix is not positive definite here")
...
#References
 ...
 # dist = numpy.linalg.norm(a - b)
        ...
        # PH.printme(PH.p1, "total squared distances", total_squared_distances)
        # kernel_val = np.exp(-((total_squared_distances ** 2) * (1 / (charac_length_scale * 2.0))))

        # PH.printme(PH.p1, data_point1.shape, data_point2.shape)
        # PH.printme(PH.p1, data_point1, data_point2)
        # PH.printme(PH.p1,  "dp**2", np.sum(data_point1 ** 2,1).reshape(-1,1))
        # PH.printme(PH.p1, "product is ", 2 * np.dot(data_point1, data_point2.T))
        # PH.printme(PH.p1, "sum:\n", np.sum(data_point2 ** 2,1).reshape(-1,1)+ np.sum(data_point2 ** 2,1).reshape(-1, 1) )

        # PH.printme(PH.p1, data_point1, data_point2)

        # PH.printme(PH.p1, "dp1\n",data_point1, "\ndp2", data_point2)
        # PH.printme(PH.p1, type(data_point1), type(data_point2))
        # PH.printme(PH.p1, "shapes", data_point1.shape, data_point2.shape)
        # summand1 = (data_point1**2).reshape(-1,1)
        # PH.printme(PH.p1, "summand:", summand1)
