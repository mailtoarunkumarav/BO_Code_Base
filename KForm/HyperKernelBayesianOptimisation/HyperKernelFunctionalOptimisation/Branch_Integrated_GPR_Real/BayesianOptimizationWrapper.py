...
class BayesianOptimizationWrapper:
    ...
    def opt_wrapper(start_time):
        ...
        if true_func == 'sin' or true_func == 'cos' or true_func == 'custom':
            ...
            #vehicle
            # len_scale_param_bounds = [[[0.7, 1], [1.8, 2]], [[0.001, 0.3], [1.8, 2]]]

            # #Iris
            # len_scale_param_bounds = [[[0.6, 0.9], [0.2, 0.3]], [[0.6, 0.9], [0.2, 0.3]]]

            # #wdbc
            # len_scale_param_bounds = [[[0.6, 1], [0.2, 0.3]], [[0.001, 0.3], [0.2, 0.3]]]

            # len_scale_func_type = ['linear', 'linear']
            # len_scale_params = np.array([[0.06, 0.01], [0.06, 0.01]])
            # len_scale_param_bounds = [[[0.1, 0.5],[0.1, 0.5]], [[-0.5, -0.3],[0.6, 0.8]]]
        ...
        # kernel_iter_types = [ 'var_l','fix_l', 'ard', 'm_ker']
        # kernel_iter_types = [ 'ard']
        kernel_iter_types = ['fix_l']
        # kernel_iter_types = ['m_ker']

        PH.printme(PH.p1, "\n\nParameters\nTrue Func", true_func, "\tDim:", number_of_dimensions, "\tobs_samp:",
        ...
        for i in range(number_of_runs):
            ...
            func_helper_obj = FunctionHelperBO(true_func)
            ...
            # #To debug the values obtained for accuracy
            # X = [[0.0818546730706902,0.016244350109588655] ,
            #      [0.5400593278542374,0.02784941350080644],[0.25003068519063215,0.035087344559027116]]

            print("..........Constructing observation model ...........")

            y = np.array([])
            obs_count=0
            for each_hyper_set in X:
                obs_count += 1
                print(".............Generating observation: ", obs_count, "...................")
                min_likeli = bay_opt_obj.func_helper_obj.get_true_func_value(each_hyper_set)
                y=np.append(y, min_likeli)

            print("..........Observation model complete...........")
            ...
            print("..........Normalising observation model complete...........")
            ...
            for index_norm in np.arange(len(y_norm)):
                ...
                init_regret.append(min(y_norm[:index_norm + 1]))

            # init_regret = []
            # for index_norm in np.arange(len(y)):
            #     init_regret.append(true_max - max(y[:index_norm + 1]))

            print ("Computing regret for initial points: ", init_regret)
        ...
        plt.title('Minimum Negative Log likelihood')
