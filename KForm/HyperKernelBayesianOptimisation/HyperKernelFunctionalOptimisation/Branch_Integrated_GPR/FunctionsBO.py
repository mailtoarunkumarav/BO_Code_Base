...
import sys
import datetime
sys.path.insert(0, 'C:\Arun_Stuff\GitHubCodes\KForm\HyperKernelBayesianOptimisation\HyperKernelFunctionalOptimisation\Branch_Krien_EVD')
...
class FunctionHelperBO:
    ...
    def __init__(self, func_type):
        ...
        if(func_type =="svm"):
            ...
            if(type == 'iris'):
                PH.printme(PH.p1, "Working with IRIS Data")
                ...
                PH.printme(PH.p1, "Working with WDBC Data")
                ...
                PH.printme(PH.p1, "Working with wine Data")
                ...
                PH.printme(PH.p1, "working with vehicle data")
    ...
    def get_true_func_value(self, x):
        if (self.true_func_type == 'sin'):
            ...
            # PH.printme(PH.p1, "Original", x[0],x[1],"\tc,g being queried is ", c, g )
            ...
            PH.printme(PH.p1, "\n\n#### Running HKFKO for the following imputs: ", x , "##################\n\n")
            timenow = datetime.datetime.now()
            stamp = timenow.strftime("%H%M%S_%d%m%Y")

            ker_opt_wrapper_obj = KernelOptimizationWrapper()
            mean_likelihood = ker_opt_wrapper_obj.kernel_wrapper(stamp, x)
