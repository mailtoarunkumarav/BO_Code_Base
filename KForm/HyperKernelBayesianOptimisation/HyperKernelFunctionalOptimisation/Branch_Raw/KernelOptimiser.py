...
class KernelOptimiser:
    ...
    def __init__(self, hyper_gaussian_object, acquisition_utility_object, number_of_subspace_selection_iterations,
        ...
        #
        self.complete_dataset_kernel_observations = np.array([]).reshape(-1,
                                                                         self.hyper_gaussian_object.number_of_samples_in_X_for_grid
                                                                         *self.hyper_gaussian_object.number_of_samples_in_X_for_grid)
    ...
    def optimise_kernel(self, run_count):
        ...
        print('Initial Grid Values selected for this run\n--- X in Grid ---\n', self.hyper_gaussian_object.X)
        ...
        for subspace_selection_count in range(self.number_of_subspace_selection_iterations):
            ...
            print("######  run: " + str(run_count) + "  subspace ", subspace_selection_count + 1, "  ######")
            ...
            for number_init_random_kernel_y_obs in range(self.number_of_init_random_kernel_y_observations):
                ...
                if bool_comp:
                    print("Plotting posterior before optimising kernel......")
                ...
                print("Generated new observation with observation_y: ", observations_y)
            ...
            for best_solution_count in range(self.number_of_iterations_best_solution):
                ...
                print("Using log marginal likelihood to compute the optimised length scale for the kernels")
                self.hyper_gaussian_object.compute_hyperparams_kernel_observations()
                print("optimised values: l = ", self.hyper_gaussian_object.char_length_scale, "  sig = ",
                      self.hyper_gaussian_object.signal_variance)

                print("pre-calculating COV matrix for kernel observations")
                ...
                print("Best weights: ", best_basis_weights)
                observations_kernel_new, observations_y_new = self.generate_observations(self.hyper_gaussian_object.current_kernel_bias,
                                                                                         best_basis_weights,
                                                                                         self.hyper_gaussian_object.current_kernel_samples)
                print("New observation found in the given subspace")
                print("New kernel suggested: ", observations_kernel_new)
                print("with observation value: ", observations_y_new)
                ...
                # print("after", self.hyper_gaussian_object.observations_y)

            print("######    search in subspace", (subspace_selection_count + 1), " completed   ######")
            ...
            print("Best solution is selected as \n", self.best_solution)
            # print("\nObservations_kernel:", self.complete_dataset_kernel_observations)
            print("Subspace: ", (subspace_selection_count+1), " completed..\n\n\n\n")
            print("Data set after:", (subspace_selection_count+1),  " subspace")
            print("Observation_y:", self.complete_dataset_y.T, "\n")
            # print("Observations_kernel: ", self.complete_dataset_kernel_observations, "\n\n")
