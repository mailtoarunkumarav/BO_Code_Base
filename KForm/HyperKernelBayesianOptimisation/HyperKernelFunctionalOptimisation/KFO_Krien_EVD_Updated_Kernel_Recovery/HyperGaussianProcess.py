...
class HyperGaussianProcess:
    ...
    def estimate_kernel_for_Xtil(self, datapointx1, datapointx2, current_observations_kernel):
        ...
        # Flipping
        # bool_negative = np.array(alpha) < 0
        # total_negative_values = np.sum(bool_negative)
        # # PH.printme(PH.p1, "@#@#Total negative values: ", total_negative_values)
        # positive_alpha = np.absolute(alpha)
    ...
    def plot_kernel_wrapper(self, current_observations_kernel, msg):
        ...
        xbound = np.linspace(0, 5, 500).reshape(-1, 1)
        X1, X2 = np.meshgrid(xbound, xbound)
        for xb_i in range(len(xbound)):
            for xb_j in range(len(xbound)):
                num = self.estimate_kernel_for_Xtil_plot(xbound[xb_i], xbound[xb_j], current_observations_kernel)
                kernel_mat[xb_i][xb_j] = num
                # Kernel Norm
        ...
        PH.printme(PH.p1, "Final Kernel matrix: ", repr(kernel_mat))
        np.savetxt("Kernel_KFO_Exp2.csv", kernel_mat, delimiter=",")
    ...
    def compute_predictive_likelihood(self, current_observations_kernel, msg):

        PH.printme(PH.p1, "Computing Predictive Likelihood: ", current_observations_kernel)
