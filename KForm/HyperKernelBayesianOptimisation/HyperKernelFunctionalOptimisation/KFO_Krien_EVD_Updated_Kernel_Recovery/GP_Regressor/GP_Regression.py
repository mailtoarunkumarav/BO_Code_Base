...
class GaussianProcess:
    ...
    def compute_predictive_loglikelihood(self, kernel_type, observations_kernel, optional_hyper_gp_obj):
        ...
        K_xs_xs = self.compute_kernel_matrix_hyperkernel(self.Xs, self.Xs, observations_kernel)
        variance_mat = K_xs_xs - np.dot(factor1.T, factor1)
        variance = np.diag(variance_mat+1e-6)

        PH.printme(PH.p1, "Mean: ", mean, "\nVariance: ", variance, "\nys: ",self.ys,"\n\n\n")

        llk_sum = 0
        for i in range(len(self.Xs)):

            term1 = (-1/(2*variance[i]))
            term2 = ((self.ys[i] - mean[i])**2)

            likeli = np.log(1/(np.sqrt(2*np.pi*variance[i])))*(np.exp(term1*term2))
            llk_sum = llk_sum + likeli

        PH.printme(PH.p1, "Predictive likelihood", llk_sum)
        return llk_sum


    def compute_predictive_loglikelihood_linear(self, kernel_type):
        ...
        for i in range(len(self.Xs)):
            ...
            term1 = (-1/(2*variance[i][i]))
            term2 = ((self.ys[i] - mean[i])**2)

            likeli = np.log(1/(np.sqrt(2*np.pi*variance[i][i])))*(np.exp(term1*term2))
            llk_sum = llk_sum + likeli

        PH.printme(PH.p1, "Predictive likelihood for ground truth linear kernel", llk_sum)
        return llk_sum
    ...
    def runGaussian(self, count, kernel_type, observations_kernel, optional_hyper_gp_obj):
        ...
        if kernel_plot and count != -1:

            PH.printme(PH.p1, "Plotting GT kernel")
            ...
            for i in range(len(x)):
                for j in range(len(x)):
                    num = self.computekernel(np.array([x[i]]), np.array([x[j]]))
                    kernel_mat[i][j] = num
                    # den1 = self.computekernel(np.array([x[i]]), np.array([x[i]]))
                    # den2 = self.computekernel(np.array([x[j]]), np.array([x[j]]))
                    # kernel_mat[i][j] = num/np.sqrt(den1*den2)

            np.savetxt("Kernel_GT_Exp2.csv", kernel_mat, delimiter=",")
            fig = plt.clf()
            fig = plt.figure(self.output_gen_time+"__"+self.kernel_type+"_Kernel")
            ax = fig.gca(projection='3d')
