...
sys.path.insert(0, 'C:\Arun_Stuff\GitHubCodes\KForm\HyperKernelBayesianOptimisation\HyperKernelFunctionalOptimisation'
                   '\KFO_Krien_EVD_Updated_Kernel_Recovery\GP_Regressor')
    ...
    def kernel_wrapper(self, start_time, input):
        ...
        for i in range(number_of_runs):
            ...
            if synthetic_data:
                posterior_plot = True
                role = "groundtruth"
                ground_truth_gp_wrapper_obj = GaussianProcessWrapper(synthetic_data)
                ground_truth_gp = ground_truth_gp_wrapper_obj .construct_gp_regressor_syn(start_time, posterior_plot, role)
                role = "KFO"
