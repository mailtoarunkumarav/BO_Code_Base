...
class BayesianOptimization:
    ...
    def run_bayes_opt(self, run_count):
        ...
        for i in range(self.no_iterations):
            PH.printme(PH.p3, "###### BO" + str(run_count) + "  iteration: ", i + 1,"    " ,self.gp_obj.kernel_char,"  ######")
            ...
            if self.gp_obj.len_scale_estimation and (i+1) % 3 == 0:
                ...
                for ind in np.arange(self.gp_obj.number_of_restarts_likelihood):
                    ...
                    # PH.printme(PH.p3, "Initial length scale: ", init_len_scale, "\nInitial variance: ", init_var)
                ...
                PH.printme(PH.p3, "Opt Length scale: ", self.gp_obj.char_length_scale, "\nOpt variance: ", self.gp_obj.signal_variance)
            ...
            if (self.gp_obj.weights_estimation and (i + 1) % 3 == 0):
                ...
                for ind in np.arange(self.gp_obj.number_of_restarts_likelihood):
                    ...
                    if (log_likelihood > log_like_max):
                        PH.printme(PH.p3, "New maximum log likelihood ", log_likelihood, " found for params ", params)
                        x_max_value = maxima['x']
                        log_like_max = log_likelihood

                if not self.gp_obj.disp_bool:
                    PH.printme(PH.p3, "!!!!!!!!!!!Matrix went non positive definite, inverting with pinv")
                self.disp_bool = True

                self.gp_obj.len_weights = x_max_value[0:4]
                self.gp_obj.signal_variance = x_max_value[len(maxima['x']) - 1]

                PH.printme(PH.p3, "Opt weights: ", self.gp_obj.len_weights, "   variance:", self.gp_obj.signal_variance)
            ...
            if (self.gp_obj.params_estimation and (i + 1) % 3 == 0):
                ...
                for ind in np.arange(self.gp_obj.number_of_restarts_likelihood):
                    ...
                    if (log_likelihood > log_like_max):
                        PH.printme(PH.p3, ind," New maximum log likelihood ", log_likelihood, " found for params ", params)
                        x_max_value = maxima['x']
                        log_like_max = log_likelihood
                ...
                PH.printme(PH.p3, "Opt Params: ", self.gp_obj.len_scale_params, "   variance:", self.gp_obj.signal_variance)
            ...
            if (self.acq_func_obj.acq_type == 'rs'):
                PH.printme(PH.p3, "performing RS for the maxima")
            ...
            PH.printme(PH.p3, "xnew:", xnew, "\noriginal",)

            # #Replacing code here to incorporate the SVM hyperparams tuning
            ...
            PH.printme(PH.p3, "(",xnew,ynew, ") is the new value added..    Original: ", (xnew_orig, ynew_orig) )
