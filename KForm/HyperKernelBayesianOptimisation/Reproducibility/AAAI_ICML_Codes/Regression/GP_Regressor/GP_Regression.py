...
class GaussianProcess:
        ...
        if 'gca_fill' in plot_params.keys():
            if len(plot_params['gca_fill']) == 3:
                ...
                if plot_params['gca_fill'][3].startswith('color'):
                    ...
                    PH.printme(PH.p4, len(plot_params['gca_fill']), color)
                    plt.gca().fill_between(plot_params['gca_fill'][0], plot_params['gca_fill'][1],
    ...
    def compute_log_marginal_likelihood_hyperkernel(self, observations_kernel):
        ...
        # Find L from K = L *L.T instead of inversing the covariance function
        # PH.printme(PH.p4, "condition number is ", np.linalg.cond(Knoise))
        ...
        try:
            ...
            K_x_x = self.compute_kernel_matrix_hyperkernel(self.X, self.X, observations_kernel)
            PH.printme(PH.p4, Knoise[0][0])
            exit(0)

            # PH.printme(PH.p4, "len", self.len_scale_params, self.signal_variance,self.X,self.y )
            # if self.disp_bool:
    ...
    def plot_kernel(self, msg, observations_kernel):
        ...
        PH.printme(PH.p4, "plotting kernel for linear kernel\n")
    ...
    def compute_predictive_likelihood(self, count, kernel_type, observations_kernel, optional_hyper_gp_obj):
        ...
        for i in range(len(self.Xs)):
            ...
            # PH.printme(PH.p4,"Likelihood: ", likeli)
            if likeli == 0:
                PH.printme(PH.p4,
                           # "\n\n\n\nMean: ", mean, "\nVariance: ", variance,
            ...
            # PH.printme(PH.p4,"prob: ", likeli)
            # PH.printme(PH.p4,"-log: ", neg_log_likelihood)
        # PH.printme(PH.p4,"sum_-log: ", neg_log_sum)
        return np.array([neg_log_sum])

    def runGaussian(self, count, kernel_type, observations_kernel, optional_hyper_gp_obj):

        PH.printme(PH.p4, "!!!!!!!!!!Gaussian Process Started!!!!!!!!!")
        ...
        if self.hyper_params_estimation:
            ...
            PH.printme(PH.p4, "Hyper Params estimating..")
            ...
            for ind in np.arange(self.number_of_restarts_likelihood):
                ...
                # PH.printme(PH.p4, "Initial length scale: ", init_len_scale, "\nInitial variance: ", init_var)
                ...
                if (log_likelihood > log_like_max):
                    PH.printme(PH.p4, "New maximum log likelihood ", log_likelihood, " found for l= ",
            ...
            PH.printme(PH.p4, "Opt Length scale: ", self.char_len_scale, "\nOpt variance: ", self.signal_variance)
        ...
        if self.weight_params_estimation:
            ...
            for ind in np.arange(self.number_of_restarts_likelihood):
                ...
                if log_likelihood > log_like_max:
                    PH.printme(PH.p4, "New maximum log likelihood ", log_likelihood, " found for params ", params)
            ...
            PH.printme(PH.p4, "Opt weights: ", self.len_weights, "   variance:", self.signal_variance)
        ...
        if self.degree_estimation:
            ...
            for deg in np.arange(1, 11):
                ...
                PH.printme(PH.p4, "deg: ", deg, log_likelihood)
                if log_likelihood > log_like_max:
                    PH.printme(PH.p4, "New maximum log likelihood ", log_likelihood, " found for degree ", deg)
                    max_degree = deg
                    log_like_max = log_likelihood

            self.degree = max_degree

            PH.printme(PH.p4, "Opt Degree: ", self.degree)

        PH.printme(PH.p4, self.signal_variance)
