...
class KernelOptimiser:
    ...
    def optimise_kernel(self, run_count):
        ...
        PH.printme(PH.p3, 'Initial Grid Values selected for this run\n--- X in Grid ---\n', self.hyper_gaussian_object.X)
        ...
        for subspace_selection_count in range(self.number_of_subspace_selection_iterations):
            ...
            PH.printme(PH.p3, "#@#@#@#@#@#@#@#@" + str(run_count) + "  subspace ", subspace_selection_count + 1, "  ######")
            ...
            for number_init_random_kernel_y_obs in range(self.number_of_init_random_kernel_y_observations):
                ...
                if bool_comp:
                    PH.printme(PH.p3, "Plotting posterior before optimising kernel......")
                ...
                PH.printme(PH.p3, "Generated new observation with observation_y: ", observations_y)
            ...
            for best_solution_count in range(self.number_of_iterations_best_solution):
                PH.printme(PH.p3, "\nTrying for best solution each iteration:" "   in subspace: ",
                           (subspace_selection_count+1))
                PH.printme(PH.p3, "Using log marginal likelihood to compute the optimised length scale for the kernels")
                self.hyper_gaussian_object.compute_hyperparams_kernel_observations()
                PH.printme(PH.p3, "optimised values: l = ", self.hyper_gaussian_object.char_length_scale, "  sig = ",
                           self.hyper_gaussian_object.signal_variance)

                PH.printme(PH.p3, "pre-calculating COV matrix for kernel observations")
                ...
                best_basis_weights = self.acquisition_utility_object.minimise_acq_function(self.hyper_gaussian_object,
                                                                                           best_solution_count+1)

                PH.printme(PH.p3, "Best weights: ", best_basis_weights)
                observations_kernel_new, observations_y_new = self.generate_observations(self.hyper_gaussian_object.current_kernel_bias,
                                                                                         best_basis_weights,
                                                                                         self.hyper_gaussian_object.current_kernel_samples)
                PH.printme(PH.p3, "New observation found in the given subspace")
                PH.printme(PH.p3, "New kernel suggested: ", observations_kernel_new)
                PH.printme(PH.p3, "with observation value: ", observations_y_new)
                ...
                # PH.printme(PH.p3, "after", self.hyper_gaussian_object.observations_y)

            PH.printme(PH.p3, "######    search in subspace", (subspace_selection_count + 1), " completed   ######")
            ...
            # Commenting to calculate negative likelihood
            # index_subspace_best_value = self.complete_dataset_y.argmax()
            index_subspace_best_value = self.complete_dataset_y.argmin()
            ...
            PH.printme(PH.p3, "Best solution is selected as \n", self.best_solution)
            # PH.printme(PH.p3, "\nObservations_kernel:", self.complete_dataset_kernel_observations)
            # PH.printme(PH.p3, "Observations_kernel: ", self.complete_dataset_kernel_observations, "\n\n")

        # PH.printme(PH.p3, "plotting results")

        # plotting kernel mean
        # size_grid = self.hyper_gaussian_object.number_of_samples_in_X_for_grid * self.hyper_gaussian_object.number_of_samples_in_X_for_grid
        # self.hyper_gaussian_object.plot_kernel_wrapper(self.hyper_gaussian_object.current_kernel_samples[0].reshape(1, size_grid),
        #                                                                                               "Kernel Samples")
