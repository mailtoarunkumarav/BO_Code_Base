...
class AcquisitionUtility():
    ...
    def upper_confidence_bound_util(self, lambda_weights, hyper_gp_obj, iteration_count):
        ...
        with np.errstate(divide='ignore') or np.errstate(invalid='ignore'):
            ...
            # PH.printme(PH.p3, "tried", lambda_weights, "obtained ", -1*result)

            # Commenting to minimise the acquisition function for calculating neg likelihood
            # return -1 * result
            return result
    ...
    def minimise_acq_function(self, hyper_gaussian_object, iteration_count):
        ...
        if self.acq_type == "UCB":
            ...
            PH.printme(PH.p3, "ACQ Function : UCB ")
            ...
            for starting_point in random_starting_points:
                ...
                if fvalue < function_best:
                    PH.printme(PH.p3, "New best Fval: ", fvalue, " found at: ", min_weights['x'])
                    # if min_weights['x'][1] == 0 and min_weights['x'][2] == 0:
                    #     PH.printme(PH.p3, "weights are zero here")
                    #     mean, variance = self.hyper_gaussian_object.predict_for_hypergp(np.array([min_weights['x']]))
                    #     std_dev = np.sqrt(variance)
                    #     fvalue = self.ucb_acq_func(mean, std_dev, iteration_count)
            ...
            PH.printme(PH.p3, "UCB Best is ", function_best, "at ", min_basis_weights)
