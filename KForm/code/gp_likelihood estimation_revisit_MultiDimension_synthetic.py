...
class GaussianProcess:
    ...
    def free_se_kernel(self, data_point1, data_point2, rand_vector1, rand_vector2, charac_length_scale, signal_variance):
        ...
        for i in np.arange(len(data_point1)):
            for j in np.arange(len(data_point2)):
                ...
                kernel_mat[i, j] = np.exp(0.5 * (1/(charac_length_scale**2)) * term2) * (signal_variance**2)
    ...
    def runGaussian(self, count, Xs, ys, X, y):
        ...
        if (self.likelihood_estimation):
            ...
            print("Opt Params: Beta:", self.beta_params, "\tlengthscale: ",self.charac_length_scale, "\tVariance:",self.signal_variance)
        ...
        plot_posterior_distr_params = {'plotnum': 'Fig 3_' + str(count),
                                       # 'axis': [self.linspacexmin, self.linspacexmax, linspaceymin, linspaceymax],
                                       'axis': [0, 1, linspaceymin, linspaceymax],
...
if __name__ == "__main__":
    ...
    number_of_GP_iterations = 3
    mean_array =[]
    rmse_array=[]
    mean = None
    count = 10
    for i in np.arange(number_of_GP_iterations):
        print("\n\nAdding observed sample ",i+1)
        gaussianObject.X = Xnew = X[:(number_of_observed_samples+i)]
        gaussianObject.y = ynew = y[:(number_of_observed_samples+i)]
        gaussianObject.number_of_observed_samples=number_of_observed_samples+i
        mean = gaussianObject.runGaussian(count, Xs, ys, Xnew, ynew).reshape(-1,1)
        mean_array.append(mean)
        rmse = (np.linalg.norm(mean-ys))/number_of_test_datapoints
