...
class GaussianProcess:
    ...
    def __init__(self, kernel_type, number_of_test_datapoints, noise, random_seed, linspacexmin, linspacexmax,
                 ...
                 number_of_restarts_likelihood, len_scale_params, beta_bounds, beta_params, likelihood_estimation):
    ...
    def free_kernel(self, data_point1, data_point2, charac_length_scale, signal_variance):
        ...
        self.charac_length_scale = 0.5

        numerator = self.free_kernel_wrapper(data_point1, data_point2, self.charac_length_scale, self.signal_variance)
        denom1 = self.free_kernel_wrapper(data_point1, data_point1, self.charac_length_scale, self.signal_variance)
        denom2 = self.free_kernel_wrapper(data_point2, data_point2, self.charac_length_scale, self.signal_variance)
        tot_denom = (np.sqrt(np.multiply(denom1, denom2)))
        tot_denom = 1
        result = numerator / tot_denom
        return result

    def free_kernel_wrapper(self, data_point1, data_point2, rand_vector1, rand_vector2):
        random_vector1 = [0.2]
        random_vector2 = [1.5]

        # data_point1 = np.array([[1, 2], [3, 4]])
        # data_point2 = np.array([[4, 5], [10, 20], [2, 3], [5, 8]])
        # random_vector1 = [-1, 1]
        # random_vector2 = [5, 5]
        # self.beta_params[0] =2
        # operand1 = np.multiply((self.beta_params[0]** 2), self.free_se_kernel( data_point1, data_point2, random_vector1, random_vector2))
        # print("##", "##",self.free_se_kernel( data_point1, data_point2, random_vector1, random_vector2), "##","\n",
        #       operand1)
        # exit(0)
    ...
    def free_se_kernel(self, data_point1, data_point2, rand_vector1, rand_vector2):
        ...
        for i in np.arange(len(data_point1)):
            for j in np.arange(len(data_point2)):
                ...
                # rand_v1_norm = np.linalg.norm(rand_vector1, axis=1)
                # rand_v2_norm = np.linalg.norm(rand_vector2, axis=1)
                # dp1_norm = np.linalg.norm(data_point1[i, :], axis =1)
                # dp2_norm = np.linalg.norm(data_point2[j, :], axis =1)

                term2 = ( 2 * each_kernel_val - ((data_point1[i, :]) **2)- ((
                              data_point2[j,:]) **2) - (np.square(rand_vector1))- (np.square(rand_vector2)))
                kernel_mat[i, j] = np.exp(0.5 * (1/(self.charac_length_scale**2)) * term2)
    ...
    def optimize_log_marginal_likelihood_l(self, input):
        ...
        self.beta_params = np.array(input)
    ...
    def runGaussian(self, count, Xs, ys, X, y):
        ...
        if (self.likelihood_estimation):
            ...
            for dim in np.arange(self.number_of_dimensions):
                random_point_each_dim = np.random.uniform(self.beta_bounds[dim][0],
                                                               self.beta_bounds[dim][1],
            ...
            for ind in np.arange(self.number_of_restarts_likelihood):
                ...
                print(ind+1, ". Initial values for beta1 & beta2 are ", beta1, beta2)
            ...
            print("Opt Params: ", self.beta_params)
...
if __name__ == "__main__":
    ...
    gaussianObject = GaussianProcess(kernel_type, number_of_test_datapoints, noise, random_seed, linspacexmin,
                                     ...
                                     beta_bounds, beta_params, likelihood_estimation)
