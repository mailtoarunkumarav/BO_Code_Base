...
class HyperGaussianProcess:
    ...
    def __init__(self, X, hyperkernel_type, kernel_type, char_length_scale, sigma_len, sigma_len_bounds, signal_variance_bounds,
                 ...
                 number_of_restarts_likelihood, no_principal_components, hyper_char_len_scale,  len_scale_params, len_scale_param_bounds):
    ...
    def len_scale_func(self, data_point):
        ...
        # c = self.len_scale_params[2]

        len_scale_values = np.array([])
        value = np.dot(a, data_point) + b

        if value <= 0:
            value = 1e-6
        len_scale_values = np.append(len_scale_values, value)

        # # Gaussian Shape length scale
    ...
    def svl_hyper(self, datapoint1, datapoint2, datapoint3, datapoint4):

        sig_var_1 = 1
        sig_var_2 = 1

        len_scale_vector_datapoint1 = self.len_scale_func(datapoint1)
        len_scale_vector_datapoint2 = self.len_scale_func(datapoint2)
        len_sq_sum1 = len_scale_vector_datapoint1 ** 2 + len_scale_vector_datapoint2 ** 2

        len_scale_vector_datapoint3 = self.len_scale_func(datapoint3)
        len_scale_vector_datapoint4 = self.len_scale_func(datapoint4)
        len_sq_sum2 = len_scale_vector_datapoint3 ** 2 + len_scale_vector_datapoint4 ** 2

        difference1 = datapoint1 - datapoint2
        squared_difference1 = np.dot(difference1, difference1.T)

        difference2 = datapoint3 - datapoint4
        squared_difference2 = np.dot(difference2, difference2.T)

        term1_num = len_scale_vector_datapoint1*len_scale_vector_datapoint2*len_scale_vector_datapoint3*len_scale_vector_datapoint4
        term1_den = len_sq_sum1 * len_sq_sum2
        term1 = 2*np.sqrt(term1_num/term1_den)
        term2 = np.exp((-1)*((squared_difference1/len_sq_sum1)+(squared_difference2/len_sq_sum2)))
        kernel_val = sig_var_1* sig_var_2 * term1 * term2
    ...
    def compute_kernel_with_hyperkernel(self, x1, x2, x3, x4):
        ...
        elif self.hyperkernel_type == 'SVL_Hyperkernel':
            covariance = self.svl_hyper(x1, x2, x3, x4)
    ...
    def compute_covariance_in_X2(self, X):
        ...
        #                 kappa_kernel_Xtil_Xtil[mat_index1][mat_index2] = self.compute_kernel_with_hyperkernel(X[index1], X[index2],
        #                                                                                                       X[index3], X[index4])
        #                 # PH.printme(PH.p1, "[" + str(mat_index1) + "][" + str(mat_index2) + "]\t" + str(X[index1][0]) + "_" + str(
        #                 #     X[index2][0]) + "_" + str(X[index3][0])
        #                 #       + "_" + str(X[index4][0]))
        ...
        for each_krien_kernel_index in range(self.number_of_basis_vectors_chosen):
            ...
            for kri_index in range(len(krien_eigen_values)):
                if krien_eigen_values[kri_index] < 0:
                    # PH.printme(PH.p1, "Negative Eigen for Krien Kernel, updating the value")
    ...
    def compute_kappa_utils(self):
        ...
        PH.printme(PH.p1, "Eigen values for the calculation of the dimensionality reduction...")
        ...
        # # plot eigen spectrum
        # val = []
        # for each_ev in eigen_values:
        #     val.append(math.log(each_ev, 10))
        #
        # plt.plot(np.linspace(1,400, 400), val)
        ...
        # #Old method to obtain principal components
        # self.princ_eig_vals_unsrt = eigen_values[(total_components - self.no_principal_components):total_components]
        # self.princ_eig_vec_unsrt = eigen_vectors[:, (total_components - self.no_principal_components):total_components]
    ...
    def generate_basis_as_kernel_samples(self):
        ...
        # old code - Non restricted tp positive values
        # Sample 3 standard normals for each of the unseen data points
        # standard_normals = np.random.normal(size=(len(self.X) * len(self.X), self.number_of_basis_vectors_chosen))

        # # Older version of generating samples commented
        # # Restricted to positive samples
        # std_norms_array = []
        # count = 0
        # total_samples = self.number_of_basis_vectors_chosen * len(self.X) * len(self.X)
        ...
        #         count = count + 1
        #
        # standard_normals = np.array(std_norms_array).reshape(len(self.X) * len(self.X), self.number_of_basis_vectors_chosen)
    ...
    def SE_Kernel_gnorm(self, data_point1, data_point2, char_length_scale, signal_variance):
        ...
        for i in np.arange(len(data_point1)):
            for j in np.arange(len(data_point2)):
                ...
                # sq_difference = np.sqrt(np.dot(alpha_difference.T, np.dot(self.kappa_kernel_Xtil_Xtil, alpha_difference)))
    ...
    def compute_hyperparams_kernel_observations(self):
        ...
        # # #Plotting code
        # len_values = np.linspace(0.1, 10, 500).reshape(500, 1)
        # likeli = []
        # for each in len_values:
        #     res = self.opt_log_likeli_kernel_l(each)
        #     likeli.append(res[0])
        # PH.printme(PH.p1, likeli)
        # plt.figure("Loglikelihood")
        # plt.plot(len_values, likeli)
        ...
        PH.printme(PH.p1, "Opt Length scale: ", self.char_length_scale, "\nOpt variance: ", self.signal_variance)
    ...
    def compute_covariance_matrix_for_kernels(self, data_point1, data_point2):
        ...
        if self.kernel_type == "SE":
            ...
            # result = self.SE_Kernel_gnorm(data_point1, data_point2, self.char_length_scale, self.signal_variance)
    ...
    def predict_for_hypergp(self, lambda_weights):
        ...
        # Commented to speed up the calculations by calculating L_K_K_hypergp once in the beginning and cache it
        # cov_K_K_hypergp = self.compute_covariance_matrix_for_kernels(self.observations_kernel, self.observations_kernel)
        # L_K_K_hypergp = np.linalg.cholesky(cov_K_K_hypergp + 1e-6 * np.eye(len(self.observations_kernel)))

        # predict without EVD of Kappa
    ...
    def estimate_kernel_for_Xtil(self, datapointx1, datapointx2, current_observations_kernel):
        ...
        for i in range(len(self.X)):
            for j in range(len(self.X)):
                ...
                # hyper_value_den1 = self.compute_kernel_with_hyperkernel(datapointx1, datapointx2, self.X[i], self.X[i])
                # hyper_value_den1 = self.compute_kernel_with_hyperkernel(datapointx1, datapointx2, datapointx1, datapointx2)
                # hyper_value_den2 = self.compute_kernel_with_hyperkernel(datapointx1, datapointx2, self.X[j], self.X[j])
                # hyper_value_den2 = self.compute_kernel_with_hyperkernel(self.X[i], self.X[j], self.X[i], self.X[j])
                # hyper_value = hyper_value_num/np.sqrt(hyper_value_den1*hyper_value_den2)
                kernel_mat = np.append(kernel_mat, hyper_value_num)
                # kernel_mat = np.append(kernel_mat, hyper_value)

        # # Commented to modify the kernel estimation with the help of Eigen values and Eigen vectors
        # # New method which calculate only once is added in GP_Regression
        # if self.pre_calculated_L_Kappa_Kobs is None:
        #     L_sol_ker_obs = np.linalg.solve(self.L_kappa_kernel_Xtil_Xtil, current_observations_kernel[0].T)
        #     self.pre_calculated_L_Kappa_Kobs = np.linalg.solve(self.L_kappa_kernel_Xtil_Xtil.T, L_sol_ker_obs)
        # # New method to speed up the computations with pre-calculated value
        # estimated_kernel_value = np.dot(kernel_mat, self.pre_calculated_L_Kappa_Kobs)

        # # Old method : less efficient as it calculates the following terms repeatedly
    ...
    def plot_kernel_wrapper(self, current_observations_kernel, msg):
        ...
        # Commented as it will be not be used with EVD of Kappa
        # # Cholesky decomposition to find L from covariance matrix K i.e K = L*L.T
        # self.L_kappa_kernel_Xtil_Xtil = np.linalg.cholesky(self.kappa_kernel_Xtil_Xtil + 1e-6 * np.eye(len(self.X) * len(self.X)))
    ...
    def estimate_kernel_for_Xtil_plot(self, datapointx1, datapointx2, current_observations_kernel):
        ...
        # Old method for kernel plotting (before EVD)
        # L_sol_ker_obs_old = np.linalg.solve(self.L_kappa_kernel_Xtil_Xtil, current_observations_kernel[0].T)
        # L_sol_ker_new_obs_old = np.linalg.solve(self.L_kappa_kernel_Xtil_Xtil, kernel_mat)
...
'''
    # Not Required
    def fit_hypergp_for_observations(self, observations_kernel, observations_y, Xs):

        PH.printme(PH.p1, "fitting observations to the kernel", observations_y)
        # cov_K_K = self.compute_covariance_matrix_for_kernels(observations_kernel, observations_kernel)
        self.estimate_kernel_for_Xtil(Xs)
        ####Unfinished work here
        # cov_Xtils_Xtils = self.compute_covariance_in_X2(Xs)
        # PH.printme(PH.p1, cov_xtils_xtils)
...
bool = False
        if len(data_point1) != len(data_point2):
            PH.printme(PH.p1, "here calcua \n", data_point2)
            bool = True
   
if bool:
    PH.printme(PH.p1, "\n\n\nkappa", self.kappa_kernel_Xtil_Xtil, "\n\n\ndp1", data_point1[i, :], "\n\nl_dp1 ", L_sol_dp1,
          "*****\n\ndp2", data_point2[j, :], "\n\nL_dp2", L_sol_dp2,
          "####\n\nalphadiff", alpha_difference,
          "\n\n\n")
