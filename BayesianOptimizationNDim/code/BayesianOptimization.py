...
class BayesianOptimization:
    ...
    def run_bayes_opt(self, run_count):
        ...
        Xs = (Xs - self.gp_obj.linspacexmin) / (self.gp_obj.linspacexmax - self.gp_obj.linspacexmin)
        ys = (ys - self.gp_obj.linspaceymin) / (self.gp_obj.linspaceymax - self.gp_obj.linspaceymin)
        ...
        # # # self.gp_obj.plot_visuals(10, Xs, ys, mean, standard_deviation, f_prior, f_post)
        ...
        for i in range(self.no_iterations):
            print("######  run: " + str(run_count) + "  iteration: ", i + 1, "   Type:  "+ self.gp_obj.kernel_char+"   ######")
            ...
            if (self.gp_obj.params_estimation and (i + 1) % 1 == 0):
                ...
                # Depending on the number of dimensions and bounds, generate random multi-starting points to find maxima
                for dim in np.arange(self.gp_obj.number_of_dimensions):
                    random_data_point_each_dim = np.random.uniform(-1,1,
                                                                   self.gp_obj.number_of_restarts_likelihood). \
                        reshape(1, self.gp_obj.number_of_restarts_likelihood)
                ...
                for ind in np.arange(self.gp_obj.number_of_restarts_likelihood):
                    ...
                    # init_points = np.append(init_points, param_c)
                    ...
                    # params = np.append(params, c_temp)
                ...
                b = x_max_value['x'][1]
                # c = x_max_value['x'][2]
                # self.gp_obj.len_scale_params = np.array([a,b,c])
            ...
            xnew_orig = xnew * (self.gp_obj.linspacexmax - self.gp_obj.linspacexmin) + self.gp_obj.linspacexmin
            ynew_orig = self.func_helper_obj.get_true_func_value(np.matrix(xnew_orig))
            ynew = (ynew_orig - self.gp_obj.linspaceymin)/ (self.gp_obj.linspaceymax - self.gp_obj.linspaceymin)
            print("ori values:", xnew_orig, ynew_orig, "\nnorm", xnew,ynew)
            ...
            #     self.gp_obj.plot_posterior_predictions(self.acq_func_obj.acq_type + str(run_count) + '_' + self.gp_obj.kernel_char +'_' +
            #                                            str(i + 1),
