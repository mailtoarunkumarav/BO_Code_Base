...
class BayesianOptimizationWrapper:
    ...
    def opt_wrapper(start_time):
        ...
        len_scale_bounds = [[0.01, 1] for nd in np.arange(number_of_dimensions)]
        ...
        # Parameters for length scale function
        a=0.1
        b=0.1
        # c=0.1
        # len_scale_params = np.array([a,b,c])
        ...
        # len_scale_param_bounds = [[-1, 1], [0.001, 1] ,[0.001, 1]]

        kernel_iter_types = ['var_l']
        # kernel_iter_types = ['ard', 'fix_l', 'var_l']
        # kernel_iter_types = ['ard', 'fix_l']
        ...
        for i in range(number_of_runs):
            ...
            gaussianObject = GaussianProcess(kernel_type, params_estimation, len_scale_estimation, char_length_scale,
                                             ...
                                             len_scale_params, len_scale_param_bounds, number_of_restarts_likelihood)
            ...
            X_norm = (X - linspacexmin)/(linspacexmax - linspacexmin)
            y_norm = (y - linspaceymin)/(linspaceymax - linspaceymin)
            true_max_norm = (true_max - linspaceymin)/(linspaceymax - linspaceymin)
