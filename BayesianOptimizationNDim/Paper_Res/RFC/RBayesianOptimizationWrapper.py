...
class BayesianOptimizationWrapper:
    ...
    def opt_wrapper(start_time):
        ...
        if true_func == 'sin' or true_func == 'cos' or true_func == 'custom':
            ...
            len_scale_func_type = ['gaussian', 'gaussian']
            len_scale_params = np.array([[0.2, 0.2], [0.1, 0.2]])
            #Wdbc
            len_scale_param_bounds = [[[0.1, 0.4], [0.2, 0.3]], [[0.01, 0.3], [0.1, 0.2]]]



            # #Bounds for Normalization
            Xmin = np.array([1,2])
            Xmax = np.array([50,200])
            ymin = 0
            ymax = 1
        ...
        for i in range(number_of_runs):
            ...
            for index_norm in np.arange(len(y)):
                init_regret.append(max(y[:index_norm + 1]))
