...
class FunctionHelper:
    ...
    def get_true_func_value(self, x):
        ...
        elif(self.true_func_type == "rfc"):

            estimator = int(round(x[0]))
            sample = int(round(x[1]))

            rfc = RandomForestClassifier(n_estimators=estimator, min_samples_leaf=sample, random_state=42)
            rfc.fit(self.D_train, self.f_train)
            accuracy = rfc.score(self.D_test, self.f_test)
            return accuracy
