...
class AcquisitionFunction():
    ...
    def plot_acquisition_function(self, count, Xs, acq_func_values, xnew,  plot_axis):

        # Set the parameters of the ACQ functions plot
        plt.figure('Acquisition Function - ' + str(count))
        plt.clf()
        plt.plot(Xs, acq_func_values)
        plt.gca().fill_between(Xs.flatten(),acq_func_values, alpha =0.5)
        plt.plot(xnew,self.fmax, marker =7, color='red', ms=15)
        plt.axis(plot_axis)
        # plt.title('Acquisition Function')
        plt.xlabel('X')
        plt.ylabel('EI')
        plt.savefig('acq'+str(count)+'.pdf', pad_inches=0, bbox_inches='tight')
