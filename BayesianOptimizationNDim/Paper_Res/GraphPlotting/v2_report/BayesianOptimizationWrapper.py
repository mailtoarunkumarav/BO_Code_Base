...
class BayesianOptimizationWrapper:
    ...
    def opt_wrapper(start_time):
        ...
        # #GCLEE
        linspacexmin = 0;linspacexmax = 10;linspaceymin = -6;linspaceymax = 1
        ...
        if true_func == 'sin' or true_func == 'cos' or true_func == 'custom':
            ...
            #test func
            Xmin = np.array([0]);Xmax = np.array([10]);ymin = -4;ymax = 4
        ...
        number_of_observed_samples = number_of_dimensions + 2
        ...
        len_scale_bounds = [[0.5, 3] for nd in np.arange(number_of_dimensions)]

        # characteristic_length_scale = np.array([1 for nd in np.arange(number_of_dimensions)])
        char_length_scale = [0.18 for nd in np.arange(number_of_dimensions)]
