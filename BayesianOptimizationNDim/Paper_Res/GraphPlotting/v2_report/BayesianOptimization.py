        ...
        for i in range(self.no_iterations):
            ...
            if(self.gp_obj.number_of_dimensions==1
                ...
                self.gp_obj.plot_posterior_predictions(self.acq_func_obj.acq_type + str(run_count) + '_' + str(i + 1)+
                                                       "_"+self.gp_obj.kernel_char+'.pdf',Xs, ys, mean, standard_deviation)
                plot_axes = [0,1, 0, 0.5]
                self.acq_func_obj.plot_acquisition_function(str(run_count) + '_' + str(i + 1), Xs, np.diagonal(acq_func_values),
                                                            xnew,  plot_axes)
            ######################
