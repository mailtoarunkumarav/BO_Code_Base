...
class FunctionHelper:
    ...
    def get_true_max(self):
        ...
        elif (self.true_func_type == 'ackley6d'):
            # self.y_true_max = 0 @  x = [0, 0, 0, 0, 0, 0]
            true_max = self.get_true_func_value(np.matrix([[0, 0, 0, 0, 0, 0]]))

        elif (self.true_func_type == 'rosenbrock5d'):
            # self.y_true_max = 0 @  x = [1, 1, 1, 1, 1, 1]
            true_max = self.get_true_func_value(np.matrix([[1, 1, 1, 1, 1]]))
    ...
    def get_true_func_value(self, x):
        if (self.true_func_type == 'sin'):
            ...
            for i in np.arange(0, d):
                inner_sum1 += (x[:, i] **2 )
                inner_sum2 += (np.cos(c*x[:, i]))
            value = -a * np.exp(-b * np.sqrt((1 / d) * inner_sum1)) - np.exp((1/d)*inner_sum2) + a + np.exp(1)
            value = (-1 * value).reshape((-1, 1))
            return value
