...
class BayesianOptimizationWrapper:
    ...
    def opt_wrapper(start_time):
        ...
        if true_func == 'sin' or true_func == 'cos' or true_func == 'custom':
            ...
            len_scale_func_type = ['gaussian', 'gaussian', 'gaussian']
            len_scale_params = np.array([[0.5, 0.2], [0.5, 0.3], [0.5, 0.3]])
            len_scale_param_bounds = [[[0.5, 0.6], [0.6, 0.8]], [[0.5, 0.6], [0.6, 0.8]], [[0.5, 0.6], [0.6, 0.8]]]
            ...
            len_scale_func_type = ['gaussian', 'gaussian', 'gaussian', 'gaussian', 'gaussian', 'gaussian']
            len_scale_params = np.array([[0.5], [0.5], [0.5], [0.5], [0.5], [0.5]])
            len_scale_param_bounds = [[[0.2, 0.8]], [[0.2, 0.8]], [[0.2, 0.8]], [[0.2, 0.8]], [[0.2, 0.8]], [[0.2, 0.8]]]
        ...
        elif (true_func == 'rosenbrock5d'):
            number_of_dimensions = 5
            rosenbrock5d_bounds = [[-2.048, 2.048] for nd in np.arange(number_of_dimensions)]
            bounds = rosenbrock5d_bounds
            epsilon1 = 0.1
            epsilon2 = 0.001
            number_of_iterations = 50

            len_scale_func_type = ['gaussian', 'gaussian', 'gaussian', 'gaussian', 'gaussian']
            len_scale_params = np.array([[0.5], [0.5], [0.5], [0.5], [0.5]])
            len_scale_param_bounds = [[[0.2, 0.8]], [[0.2, 0.8]], [[0.2, 0.8]], [[0.2, 0.8]], [[0.2, 0.8]]]
        ...
        len_scale_bounds = [[0.1, 1] for nd in np.arange(number_of_dimensions)]

        # characteristic_length_scale = np.array([1 for nd in np.arange(number_of_dimensions)])
        char_length_scale = [0.1 for nd in np.arange(number_of_dimensions)]
        ...
        multi_len_scales = [0.7,0.5,0.3,0.1]
        len_weights = [1,1,1,1]
        ...
        print("\n\nParameters\nTrue Func", true_func, "\tDim:", number_of_dimensions, "\tobs_samp:", number_of_observed_samples,
              ...
              number_of_test_datapoints,"\tlen_weights: ",len_weights,"\tmulti_len_scales", multi_len_scales,"\ts:0.8*1+b:0.7\n\n")
