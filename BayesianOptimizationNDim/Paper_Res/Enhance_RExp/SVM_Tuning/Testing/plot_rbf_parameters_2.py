...
C_range = np.logspace(-5, 5, 7)
gamma_range = np.logspace(-8, -1, 6)
# C_range = np.linspace(1e-15, 1e5, 50)
# gamma_range = np.linspace(1e-5, 1e0, 50)
#
# esti_range = np.linspace(1,100,100)
# split_range = np.linspace(2,200,200)
...
C_range = np.linspace(0, 5, 40)
gamma_range = np.linspace(-8, -3, 40)

c, g = np.meshgrid(C_range, gamma_range)
print(c.shape)
y = np.zeros((40,40))
for i in range(c.shape[0]):
    for j in range(c.shape[1]):
        ...
        # c1 = ((c[i][j]))
        # g1 = (g[i][j])
        svclassifier = SVC(kernel='rbf', C=c1, gamma=g1)
...
surf = ax.plot_surface(c, g, y, rstride=1, cstride=1,cmap='viridis', linewidth=1, antialiased=False)
