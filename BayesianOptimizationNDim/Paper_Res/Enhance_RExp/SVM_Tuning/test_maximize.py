...
# C_range = np.linspace(1e-5, 1e5, 50)
...
for i in range(c.shape[0]):
    for j in range(c.shape[1]):
        svclassifier = SVC(kernel='rbf', C=c[i][j], gamma=g[i][j])
...
# surf = ax.plot_surface(c, g, y, rstride=1, cstride=1,
...
for i in range(X1.shape[0]):
    for j in range(X1.shape[1]):
        y[i][j] = np.sin(X1[i][j]) * ((np.sin(((X1[i][j]**2)/np.pi))) ** 20) + np.sin(X2[i][j]) * ((np.sin((2*(X2[i][j]**2)/np.pi))) ** 20)
...
#     for X2 in x2:
#         # print("X1",X1, "  X2",X2)
#         yy = np.sin(X1) * ((np.sin(((X1 ** 2) / np.pi))) ** 20) + np.sin(X2) * ((np.sin((2 * (X2 ** 2) / np.pi))) ** 20)
#         if yy > maxx:
#             maxx = yy
