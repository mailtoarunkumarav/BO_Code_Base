...
class FunctionHelper:
    ...
    def __init__(self, func_type):
        ...
        if(func_type =="svm" or func_type == "elastic_net"):
            # #Iris data
            ...
            if(type == 'iris'):
                ...
                print("working with vehicle data")
    ...
    def get_true_func_value(self, x):
        if (self.true_func_type == 'sin'):
            ...
            svclassifier = SVC(kernel='rbf', C=x[0], gamma=x[1])
            svclassifier.fit(self.D_train, self.f_train)
            accuracy = svclassifier.score(self.D_test, self.f_test)

            return accuracy

        elif(self.true_func_type == "elastic_net"):

            alpha = (10**x[0])
            l1r = x[1]
            # alpha = x[0]
            # l1r = x[1]
            # print("Alpha and L1Ratio queried for ", alpha, l1r)
            sgclassifier = SGDClassifier(alpha=alpha, l1_ratio=l1r, penalty='elasticnet', max_iter=3000, random_state=42)
            sgclassifier.fit(self.D_train, self.f_train)
            accuracy = sgclassifier.score(self.D_test, self.f_test)
            return accuracy
