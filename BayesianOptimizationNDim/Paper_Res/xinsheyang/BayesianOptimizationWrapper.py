...
class BayesianOptimizationWrapper:
    ...
    def opt_wrapper(start_time):
        ...
        # Benchmark 1D e^(-x)sin2pix
        # linspacexmin = 0;linspacexmax = 4;linspaceymin = -1;linspaceymax = 1

        # Benchmark 1D e^(-x)sin2pix
        linspacexmin = -10;linspacexmax = 10;linspaceymin = -2;linspaceymax = 2
        ...
        if true_func == 'sin' or true_func == 'cos' or true_func == 'custom':
            ...
            len_scale_param_bounds = [[[0.3, 0.7], [0.1, 0.3]]]
            ...
            # BEnchmark 1D e^(-x) sin2pix
            # Xmin = np.array([0]);Xmax = np.array([4]);ymin = -1;ymax = 1

            #Xinsheyang3 Function
            Xmin = np.array([-10]);Xmax = np.array([10]);ymin = -2;ymax = 2
            ...
            len_scale_param_bounds = [[[2.5, 3.5], [3, 3], [0.55,0.55]], [[2.5, 3.5], [3, 3], [0.55,0.55]]]
            ...
            number_of_iterations = 30
            len_scale_func_type = ['gaussian', 'gaussian']
            len_scale_params = np.array([[0.5, 0.17], [0.5, 0.17]])
            len_scale_param_bounds = [[[0.5, 0.55], [0.17, 0.2]], [[0.5, 0.55], [0.17, 0.2]]]
        ...
        char_length_scale = [0.8 for nd in np.arange(number_of_dimensions)]
        ...
        multi_len_scales = [0.7,0.4,0.3,0.2]
        len_weights = [1,1,1,1]
        ...
        print("\n\nParameters\nTrue Func", true_func, "\tDim:", number_of_dimensions, "\tobs_samp:", number_of_observed_samples,
              ...
              number_of_test_datapoints,"\tlen_weights: ",len_weights,"\tmulti_len_scales", multi_len_scales,"\tp:-1+b:1.1","\n\n")
            ...
            for iter_type in kernel_iter_types:
                if (iter_type == 'ard'):
                    ...
                    print("\n\nTotal EI Multi Regret\n", ei_multi_l_regret, "\n\nMulti Regret Mean", ei_multi_regret_mean,
                          "\n\nMulti Regret Deviation_std_err\n", ei_multi_regret_std_dev)
