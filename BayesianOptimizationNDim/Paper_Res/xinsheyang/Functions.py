...
class FunctionHelper:
    ...
    def get_true_max(self):
        ...
        if (self.true_func_type == 'custom'):
            ...
            # Xinsheyang Function (np.exp(-(x/beta)**(2*m)) - 2*np.exp(-(x)**2)*(np.cos(x))**2)
    ...
    def get_true_func_value(self, x):
        if (self.true_func_type == 'sin'):
            ...
            # Benchmark Function exp(-x)*sin(2.pi.x)(maxima), -exp(-x)*sin(2.pi.x) (minima) [0,4]
            ...
            # Xin-sheyang-1 * (np.exp(-(x/beta)**(2*m)) - 2*np.exp(-(x)**2)*(np.cos(x))**2) for maxima, max =1 at x = 0
            beta = 15
            m = 5
            return -1 * (np.exp(-(x / beta) ** (2 * m)) - 2 * np.exp(-(x) ** 2) * (np.cos(x)) ** 2)
