...
class BayesianOptimization:
    ...
    def run_bayes_opt(self, run_count):
        ...
        # self.gp_obj.plot_prior_samples(self.gp_obj.kernel_char+'_prior',Xs,f_prior)
        # self.gp_obj.plot_posterior_samples(self.gp_obj.kernel_char+'_posterior',Xs,f_post)
        ...
        for i in range(self.no_iterations):
            ...
            if (self.gp_obj.params_estimation and (i + 1) % 1 == 0):
                ...
                for d in np.arange(self.gp_obj.number_of_dimensions):
                    ...
                    if self.gp_obj.len_scale_func_type[d] == 'linear' :
                ...
                for ind in np.arange(self.gp_obj.number_of_restarts_likelihood):
                    ...
                    # maxima = opt.minimize(lambda x: -self.func(x),
                    ...
                    # print("current loglikelihood", log_likelihood, "\ninit: ",tot_init_points,"\nnew params ", params,"\n####\n\n")
                    # log_likelihood = self.func(params)
                    if (log_likelihood > log_like_max):
                        print("New maximum log likelihood ", log_likelihood, " found for params ", params)
                        x_max_value = maxima['x']
                        log_like_max = log_likelihood

                print("max: ", x_max_value, log_like_max)
                ...
                for type in self.gp_obj.len_scale_func_type:
                    if(type == 'linear'):
                        ...
                        total_len_scale_params.append(x_max_value[param_count:(param_count+1)])
                        # param_count += 2
                        param_count += 1
                    elif(type == 'quadratic'):
            ...
            print("(",xnew,ynew, ") is the new value added..")
            ...
            # mean, diag_variance, f_prior, f_post = self.gp_obj.gaussian_predict(Xs)
            # standard_deviation = np.sqrt(diag_variance)
            # if(self.gp_obj.number_of_dimensions==1 and (i == self.no_iterations-1 or i ==0)):
